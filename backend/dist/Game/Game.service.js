...
class GameService {
    ...
    async createGame(data) {
        try {
            ...
            const GameData = (0, validate_1.validateZod)(Game_model_1.GameSchema, gameData);
    ...
    async updateGame(id, data) {
        try {
            ...
            const GameData = (0, validate_1.validateZod)(Game_model_1.GameSchema, data);
            const updatedGame = await Game_repo_1.default.update(id, GameData);
            if (GameData.icon && existingGame.icon && GameData.icon !== existingGame.icon) {
                await (0, deleteVercelImage_1.deleteVercelImage)(existingGame.icon);
            }
            if (GameData.background && existingGame.background && GameData.background !== existingGame.background) {
